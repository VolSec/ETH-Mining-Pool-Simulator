...
import numpy as np
import volsec_tools
from aminingpoolsimulator.miner import HonestMiner, CostMaliciousMiner, SpiteMaliciousMiner
...
class MiningPool:
    ...
    def __init__(self, num_honest_miners, num_malicious_miners,
                 ...
                 shares_per_second=1, attack_type='COST', seed=None, attack_params=None):
    ...
    def find_all_blocks(self):
        ...
        while num_blocks_found != self.num_blocks_to_sim:
            if round_num == self.block_rounds[block_rounds_index]:
                block_rounds_index += 1
                num_blocks_found += 1
                if num_blocks_found % 500 == 0:
                    self.logger.info("NUM BLOCKS FOUND: {0}".format(num_blocks_found))
    ...
    def debug_print_scoreboard(self):
        ...
        print(tabulate(all_data, headers=["Id", "Shares", "HashRate", "Blocks Won", "Type"]))
    ...
    def write_final_stats(self):
        ...
        with open(file_name, 'w') as ftw:
            ...
            for miner in sorted_honest_miners:
                miner_output = "H,{0},{1},{2}\n".format(miner.hash_rate,
                                                   miner.shares,
                                                   miner.blocks_won)
